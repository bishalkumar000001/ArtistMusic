    ...
    async def play_media(
        ...
        try:
            ...
            if not seek_time:
                ...
                try:
                    rich_sent = await send_player_message(
                    ...
                    media.message_id = rich_sent.id
                except Exception as rich_error:
                    logger.error(f"Rich player send failed for {chat_id}: {rich_error}")
                    # Fallback keeps playback working if an old Telegram client/API
                    # endpoint is temporarily unavailable.
                    keyboard = buttons.controls(chat_id, timer=timer_text) if timer_text else buttons.controls(chat_id)
