...
from Elevenyts.helpers.rich_player import edit_player_message
    ...
    async def update_chat_timer(chat_id):
        ...
        while True:
            try:
                ...
                base_html = _lang["play_media"].format(
                    media.url, media.title, media.duration, media.user
                )
                # Rich Message buttons are part of the message content, so update the
                # whole rich message instead of replacing an inline keyboard.
                await edit_player_message(
                    chat_id, message_id, base_html, media,
                    timer=timer_text, playing=True, remove=remove,
                )
            except Exception as e:
                error_str = str(e)
                        ...
                        if alone_duration >= LEAVE_TIMEOUT:
                            ...
                            try:
                                ...
                                if current_media and current_media.message_id:
                                    base_html = _lang["play_media"].format(
                                        current_media.url, current_media.title,
                                        current_media.duration, current_media.user
                                    )
                                    await edit_player_message(
                                        chat_id, current_media.message_id, base_html,
                                        current_media, playing=False, remove=True
                                    )
